...
export const userSignup = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        res.clearCookie(COOKIE_NAME, { path: "/", domain: "tesis-backend-xqew.onrender.com", httpOnly: true, signed: true })

        const token = createToken(user._id.toString(), user.email, "7d")
        const expires = new Date()
        expires.setDate(expires.getDate() + 7)
        res.cookie(COOKIE_NAME, token, { path: "/", domain: "tesis-backend-xqew.onrender.com", expires, httpOnly: true, signed: true })
