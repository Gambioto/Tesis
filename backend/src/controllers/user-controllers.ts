...
export const userSignup = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        res.clearCookie(COOKIE_NAME, { path: "/", domain: window.location.hostname, httpOnly: true, signed: true })

        const token = createToken(user._id.toString(), user.email, "7d")
        const expires = new Date()
        expires.setDate(expires.getDate() + 7)
        res.cookie(COOKIE_NAME, token, { path: "/", domain: window.location.hostname, expires, httpOnly: true, signed: true })
